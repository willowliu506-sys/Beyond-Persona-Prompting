...
本研究围绕一个核心的 learner-representation 问题展开：当某些 learner characteristics 预期通过学习过程影响 information processing、knowledge formation 和 downstream behaviour 时，它们是否应当仅作为 persona-level descriptions 直接约束模型输出，还是可以被表示为作用于相应 functional process layer 的显式 processing constraints。以 ADHD-related Attention 和 Working-Memory characteristics 为受控案例，本文比较了 persona prompting 与 Cognitive-Process-Based（CPB）representation，并对 Main Research Question 给出了有条件的肯定回答。

实验结果表明，CPB 能够将预定义的 Attention 与 Working-Memory constraints 转化为可归因的 information-state 和 knowledge-state changes，并进一步形成与 controlled distraction 和 instructional processing demand 方向一致的 task-sensitive behavioural patterns。相比之下，persona prompting 虽然能够显式指定不同 learner profiles，但这些 persona-level differences 并未稳定转化为相同类型的 process-sensitive effects。进一步地，在加入 Language Ability 和 Big-Five response-stage prompts 后，CPB 的具体 assessment outcomes 可以发生变化，但其既有的 graded constraint structure 以及 distraction- and processing-demand-related aggregate patterns 仍然保持可观察。这说明，允许 response-level variation 与保持 process-related behavioural structure 并不矛盾：不同 functional dimensions 可以影响 simulated learner 的不同层面，而不必全部由一个 monolithic persona prompt 统一规定。

本文的主要贡献因此首先体现在 **functional learner representation**。CPB 将部分 process-relevant learner characteristics 从“学生应该怎样表现”的描述性指令中分离出来，使其通过 learning-process constraints 改变 learner state，并由 resulting state 进一步影响 downstream behaviour。其次，本文建立了与这种 representation 相匹配的 validation framework，通过 mechanism execution、information-state transitions、assessment outcomes 和 process-sensitive behavioural patterns 分层验证不同类型的 claim。Traceability 和 explicit knowledge state 在这里并非研究目的本身，而是 process-based representation 所带来的方法学优势，使 behaviour 与其形成过程之间的对应关系能够被检查，而不仅依赖最终回答进行反推。

这些结论仍受到明确的 scope 限制。ADHD cognition 远比本文操作化的 Attention 和 Working Memory 复杂，两种 mechanisms 都是为了 experimental control 而构建的有限 functional abstractions，并不等同于真实人类认知过程；PDB 也只是基于 surprisal 构建的 instructional processing-demand proxy，而非 human cognitive load 的直接测量。此外，本研究只在英文 finance materials、多轮教学和 short-answer assessment 场景中进行验证，也没有使用真实 ADHD learner data 来建立 external behavioural validity。因此，未来工作应进一步利用真实 ADHD 与 NT learner data 检验 distraction sensitivity、processing-demand sensitivity 及其 micro-to-macro behavioural patterns，并逐步扩展更真实的 probabilistic attention allocation、partial retention、interference 和 dynamic processing mechanisms，同时验证其在不同学科、任务形式和基础模型上的适用性。

总体而言，本研究并不主张以 CPB 全面取代 persona prompting。更重要的结论是：**learner representation 应与 learner characteristic 预期发挥作用的 functional level 相匹配。** 当一个 characteristic 主要影响语言表达或互动风格时，persona prompting 可能已经足够；但当它预期通过 learning process 改变 knowledge acquisition 和 subsequent task behaviour 时，将其表示在对应的 processing layer，并让 behavioural differences 从 resulting learner state 中产生，提供了一条比单纯描述“学生是什么样”更具理论对应性的模拟路径。这一方向也为未来构建更加 modular、functionally grounded 的 LLM simulated learners 提供了一个可进一步验证的研究框架。
